...
def tokenize_batch(batch):
    # Добавляем префикс "translate English to Russian: "
    prefixed_articles = ["translate English to Russian: " + sample["en"] for sample in batch["translation"]]

    # Токенизация статьи
    en_words_tokens = tokenizer(
        prefixed_articles,
        truncation=True,
        max_length=200,
        padding="max_length"
    ...
    # Токенизация резюме
    ru_words_tokens = tokenizer(
        [sample["ru"] for sample in batch["translation"]],
        truncation=True,
        max_length=200,
        padding="max_length"
    )

    labels = ru_words_tokens["input_ids"]
    ...
    return {
        "input_ids": en_words_tokens["input_ids"],
        "attention_mask": en_words_tokens["attention_mask"],
        "labels": labels_with_ignore_index # <--- Используем обработанные labels
    }
...
if __name__ == '__main__':
    ...
    dataset = raw_dataset.map(tokenize_batch, batched=True)
